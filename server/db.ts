...
import { InsertUser, users, patients, emergencyProfiles, responders, facilities } from "../drizzle/schema";
...
export async function createPatient(phone: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");

  const result = await db.insert(patients).values({ phone });
  return result;
}

/**
 * Emergency Profile database helpers
 */
export async function getEmergencyProfileByPatientId(patientId: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
export async function getEmergencyProfileById(profileId: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
...
/**
 * Responder database helpers
 */
export async function getResponderByBadgeId(badgeId: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  ...
  const result = await db
    .select()
    .from(responders)
    .where(eq(responders.badgeId, badgeId))
    .limit(1);
